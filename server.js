...
const emitUpdate = (action, payload = {}) => {
    io.emit(action, payload);
};
const emitRefresh = () => {
    io.emit('dataUpdated', { action: 'refresh' });
};
                    ...
                    if (bib) {
                        ...
                        if (hasHardConflict || membersToInsert.length === 0) {
                            results.push({
                                ...
                                duplicate: 0,
                                log: hasHardConflict
                                    ? `❌ Kihagyva: ${membersToInsert.length > 0 ? membersToInsert[0].name : 'Ismeretlen'} - Az 5Próba azonosító egy másik névhez tartozik!`
                                    : null,
                            });
                            continue;
                        }
...
// --- LÁTOGATOTTSÁGI STATISZTIKA (VISITOR ANALYTICS) ---
const analyticsFilePath = path.join(__dirname, 'backend', 'data', 'pageviews.json');
...
if (!fs.existsSync(path.join(__dirname, 'backend', 'data'))) {
    fs.mkdirSync(path.join(__dirname, 'backend', 'data'));
}
...
function savePageviews() {
    try {
        fs.writeFileSync(analyticsFilePath, JSON.stringify(pageviews, null, 2), 'utf8');
    } catch (err) {
        console.error('Hiba az analytics mentésekor:', err);
    }
}
...
app.get('/api/analytics/stats', authenticateAdmin, (req, res) => {
    try {
        ...
        // Időintervallumok meghatározása (nap, hét, hónap, év)
        ...
        // Átlagos oldalmegtekintések és konverziós adatok
        ...
        res.json({
            ...
            stats: {
                ...
                visitorDetails: visitorDetails.reverse().slice(0, 30), // utolsó 30 látogató
            },
        });
    } catch (err) {
