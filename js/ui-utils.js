...
export function formatMemberListHtml(racer) {
    ...
    if (t) {
        ...
        html += rm.map(m => `<div style="margin-bottom:6px;"><strong onclick="window.raceManager.openEditModal('${racer.id}')" style="cursor: pointer; color: var(--accent-primary); text-decoration: underline;" title="Versenyző/Csapat szerkesztése">${m.name || '?'}</strong> <br><span style="font-size:0.75rem; color:#aaa;">Szül: ${m.birth_date || '?'} | 5P: ${m.otproba_id || '-'}</span></div>`).join('');
        html += `</div></details>`;
    } else {
        html += rm.map(m => `<div style="margin-bottom:2px;"><strong onclick="window.raceManager.openEditModal('${racer.id}')" style="cursor: pointer; text-decoration: underline;" title="Versenyző szerkesztése">${m.name || '?'}</strong> <span style="font-size:0.7rem; color:#888;">(${m.birth_date || '?'})</span></div>`).join('');
    }
